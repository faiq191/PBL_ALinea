...
<body class="relative min-h-screen flex items-center justify-center bg-p3r-base font-sans overflow-hidden">
    ...
    <svg class="absolute bottom-0 left-0 w-full h-[65vh] z-0 pointer-events-none opacity-80"
    ...
    <div class="relative z-10 w-full max-w-sm p-8 text-white flex flex-col items-center">
        ...
        <div
            class="w-20 h-20 border-2 border-white rounded-full flex items-center justify-center mb-4 bg-[#153966]/30 backdrop-blur-sm">
        ...
        <form method="POST" action="/register" class="w-full" novalidate>
            @csrf

            @if (session('error'))
                <div
                    class="w-full bg-red-500/20 border border-red-500/50 text-red-100 px-4 py-3 rounded-xl mb-6 text-sm text-center backdrop-blur-sm shadow-md">
                    {{ session('error') }}
                </div>
            @endif
            ...
            @if (session('status'))
                <div
                    class="w-full bg-green-500/20 border border-green-500/50 text-green-100 px-4 py-3 rounded-xl mb-6 text-sm text-center backdrop-blur-sm shadow-md">
                    {{ session('status') }}
                </div>
            @endif
            ...
            @if ($errors->any())
                <div
                    class="w-full bg-red-500/20 border border-red-500/50 text-red-100 px-4 py-3 rounded-xl mb-6 text-sm backdrop-blur-sm shadow-md">
            ...
            <div class="mb-6 w-full">
                <div class="relative flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 absolute left-1 text-white" fill="none"
