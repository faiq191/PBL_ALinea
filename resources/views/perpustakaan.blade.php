                    ...
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div x-data="{ open: false }">
                            ...
                            <div x-show="open" x-transition x-cloak
                                ...
                                <div class="bg-white p-6 rounded-2xl w-full max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto"
                                    ...
                                    <div class="space-y-6">
                                        <div>
                                            <label
                                                class="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Genre</label>
                                            <div class="grid grid-cols-3 gap-2">
                                                @foreach ($genres as $g)
                                                    <label
                                                        class="flex items-center gap-2 text-[#4b3b3b] text-sm cursor-pointer">
                                        ...
                                        <div class="grid grid-cols-3 gap-6">
                                            <div>
                                                <label
                                                    class="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Tipe</label>
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Demografis</label>
                                                <div class="space-y-2 max-h-40 overflow-y-auto">
                                                    @foreach ($demographics as $d)
                                                        <label
                                                            class="flex items-center gap-2 text-[#4b3b3b] text-sm cursor-pointer">
                                                            <input type="checkbox" name="demo_ids[]"
                                                                value="{{ $d->id }}"
                                            ...
                                            <div>
                                                <label
                                                    class="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Tahun
                                                    Rilis</label>
                                                <div class="space-y-2 max-h-40 overflow-y-auto">
                                                    @foreach ($years->take(10) as $y)
                                                        <label
                                                            class="flex items-center gap-2 text-[#4b3b3b] text-sm cursor-pointer">
                                                            <input type="checkbox" name="year_ids[]"
                                                                value="{{ $y->id }}"
                            ...
                            <x-book-card :id="$book->id" :image="$book->image" :title="$book->title" :author="$book->author"
                                :genre="$book->genres
                                    ->map(
                                        fn(
                                            $g,
                                        ) => '<span class=\'bg-[#4b3b3b] text-white text-[10px] px-3 py-1 rounded-full mr-1\'>' .
                                            $g->name .
                                            '</span>',
                                    )
                                    ->join('')" :owner-id="$book->user_id" :is-google-api="$book->is_google_api"
                                ...
                                @foreach ($genreBooks as $book)
                                    <x-book-card :id="$book->id" :image="$book->image" :title="$book->title" :author="$book->author"
                                        :genre="$book->genres
                                            ...
                                            ->join('')" :owner-id="$book->user_id"
                                        :is-google-api="$book->is_google_api" :google-url="$book->google_url ?? '#'" />
                                @endforeach
                            </div>
                        </div>
                    @endforeach
