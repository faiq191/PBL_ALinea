...
<body class="bg-[#2c2c2c] min-h-screen" x-data="{ mode: '{{ old('source_mode', 'manual') }}' }">
        ...
        <div class="max-w-2xl w-full bg-[#e6ddd6] rounded-3xl p-8 shadow-xl">
            ...
            <div class="flex gap-2 mb-8 bg-white/50 p-2 rounded-2xl">
                ...
                <button @click="mode = 'google'" 
                    :class="mode === 'google' ? 'bg-[#5a3e3e] text-white' : 'text-[#5a3e3e]'"
                    class="flex-1 py-2 rounded-xl text-xs font-bold transition">
                    Cari Google Books
                </button>
            </div>
            ...
            <form action="/books" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div x-show="mode === 'existing'" x-transition>
                    ...
                    <select name="existing_book_id" class="w-full px-4 py-3 rounded-xl bg-white outline-none focus:ring-2 focus:ring-[#5a3e3e] @error('existing_book_id') ring-2 ring-red-500 @enderror">
                ...
                <div x-show="mode === 'google'" x-data="{ query: '', results: [], selected: null }" x-transition class="space-y-4">
                    <label class="block text-sm font-bold text-[#4b3b3b]">Cari di Google Books</label>
                    <div class="flex gap-2">
                        ...
                        <button type="button" @click="if(query) fetch(`/google-books/search?q=${query}`).then(r => r.json()).then(d => results = d)" class="bg-[#5a3e3e] text-white px-5 py-2 rounded-xl font-bold text-sm">Cari</button>
                    </div>
                    ...
                    <div class="bg-white rounded-xl divide-y divide-gray-100 max-h-60 overflow-y-auto" x-show="results.length > 0">
                        <template x-for="book in results" :key="book.id">
                            <div @click="selected = book.volumeInfo; $refs.googleVolId.value = book.id; results = []" class="p-3 hover:bg-gray-50 cursor-pointer flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-bold text-sm text-[#2c2c2c]" x-text="book.volumeInfo.title"></p>
                    ...
                    <div class="bg-white/60 p-4 rounded-xl flex items-center gap-4" x-show="selected">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-bold text-[#4b3b3b]" x-text="selected?.title"></p>
                ...
                <div x-show="mode === 'manual'" x-transition class="space-y-6">
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="title" :required="mode === 'manual'" value="{{ old('title') }}"
                                class="w-full px-4 py-2 rounded-xl bg-white border-none outline-none focus:ring-2 focus:ring-[#5a3e3e] @error('title') ring-2 ring-red-500 @enderror">
                            @error('title')
                                <p class="text-red-600 text-xs mt-1 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="text" name="author" :required="mode === 'manual'" value="{{ old('author') }}"
                                class="w-full px-4 py-2 rounded-xl bg-white border-none outline-none focus:ring-2 focus:ring-[#5a3e3e] @error('author') ring-2 ring-red-500 @enderror">
                            @error('author')
                                <p class="text-red-600 text-xs mt-1 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                            class="w-full px-4 py-2 rounded-xl bg-white border-none outline-none focus:ring-2 focus:ring-[#5a3e3e]">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-600 text-xs mt-1 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
