                    ...
                    @auth
                        ...
                        @if(!isset($book->is_google_api) || !$book->is_google_api)
                            @if(auth()->id() === $book->user_id || auth()->user()->is_admin)
                                ...
                                {{-- Tombol Hapus Buku --}}
                                <form action="/books/{{ $book->id }}" method="POST" class="inline-flex" onsubmit="return confirm('Apakah Anda yakin ingin menghapus buku ini? Tindakan ini akan menghapus data secara permanen.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-500 hover:bg-red-600 text-white font-bold px-6 py-2.5 rounded-xl shadow-md transition flex items-center gap-2">
