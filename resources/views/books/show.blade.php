...
<body class="bg-[#2c2c2c] min-h-screen font-sans">
    ...
    <div class="p-8 flex justify-center">
        <div class="max-w-6xl w-full bg-[#e6ddd6] p-8 md:p-10 rounded-3xl shadow-xl flex flex-col md:flex-row gap-10 text-[#4b3b3b]">

            <div class="w-full md:w-1/3 lg:w-1/4 shrink-0">
                <img src="{{ asset('storage/' . $book->image) }}" class="w-full aspect-[2/3] object-cover rounded-xl shadow-md mb-8">
                        ...
                        @forelse($otherOwners as $otherBook)
                            <div class="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-[#5a3e3e] flex items-center justify-center text-white font-bold">
                                ...
                                @if($otherBook->isAvailable())
                                    <form action="/loans/{{ $otherBook->id }}" method="POST">
                                        @csrf
                                        <button type="submit" class="bg-[#5a3e3e] text-white text-xs px-4 py-2 rounded-lg font-bold">
                                            Request Pinjam
                                        </button>
                                    </form>
                                @else
