                            ...
                            <div class="bg-white/40 p-5 rounded-2xl border border-white/50 shadow-inner flex flex-col sm:flex-row gap-6 items-center">
                                ...
                                <div class="flex-shrink-0 relative group">
                                    ...
                                    <template x-if="cover_source === 'url' && imageUrl">
                                        <img :src="imageUrl" x-on:error="$el.src = 'https://placehold.co/100x140?text=Error'" class="w-24 h-32 object-cover rounded-xl shadow-md border-2 border-white transition group-hover:scale-105 duration-300">
                                    </template>
                                ...
                                <div class="flex-1 w-full space-y-4">
                                    ...
                                    <div x-show="cover_source === 'file'" x-transition class="space-y-2">
                                        <div class="relative flex items-center justify-center border border-dashed border-gray-300 hover:border-[#1a3a5c] bg-white/50 rounded-xl p-4 transition cursor-pointer group">
                                            <input type="file" name="image" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" id="disc-image-input" @change="handleFileChange($event); document.getElementById('disc-file-chosen').textContent = $event.target.files[0]?.name || 'Pilih file gambar...'">
                                            <div class="text-center space-y-1.5 pointer-events-none">
                                                ...
                                                <p id="disc-file-chosen" class="text-xs text-gray-500 font-medium group-hover:text-[#1a3a5c] transition">Pilih file gambar...</p>
                                            </div>
                                        </div>
                                        @error('image')
